...
// npm install --save bip39 ethereumjs-wallet ethereumjs-tx ethereumjs-units ethereumjs-util ethereumjs-abi
// npm uninstall --save bip39 ethereumjs-wallet ethereumjs-tx ethereumjs-units ethereumjs-util ethereumjs-abi
...
export default class Ethereum {
  ...
  fromPrivateKey (hexPK) {
    ...
    this._wallet = Wallet.fromPrivateKey(key)
  }
  ...
  createRawNormalTx (txParams) {
    console.log('createRawNormalTx with txParams: ', txParams)
        
    var transaction = new Transaction()

    var defaultGasLimit = 600000
    var defaultGasPrice = 20000000000

    var gasLimit = parseInt(txParams.gasLimit, 10)
    var gasPrice = parseInt(txParams.gasPrice, 10)
  ...
  createRawSmartContractTx (txParams, inputData) {
    console.log('createRawSmartContractTx => txParams: ', txParams)
    console.log('createRawSmartContractTx => inputData: ', inputData)
    var transaction = new Transaction()
    var defaultGasLimit = 600000
    var defaultGasPrice = 20000000000
    var gasLimit = parseInt(txParams.gasLimit, 10)
    ...
    transaction.nonce = txParams.nonce
    transaction.value = txParams.value || 0
    if (inputData) {
    ...
    transaction.sign(new Buffer(txParams.privateKey, 'hex'), 'hex')
    return '0x' + transaction.serialize().toString('hex')
  }
  ...
  /**
     ...
     */
  getFunctionSmartContract (nameFunction, typeParams, Params, isBuffer = false) {
    return (isBuffer)
      ? Buffer.concat([Abi.methodID(nameFunction, typeParams), Abi.rawEncode(typeParams, Params)])
      : Abi.methodID(nameFunction, typeParams).toString('hex') + Abi.rawEncode(typeParams, Params).toString('hex')
  }
